    ...
    <section class="content">
      ...
      <div class="box">
        ...
        <div class="box-body table-responsive">
          <div class="col-md-12">
            <legend><span class="fa fa-database"></span> Daftar Masukan/Pertanyaan Help Desk</legend>
            <table id="example1"  class="table table-bordered table-striped">
              <thead>
                <tr>
                  <th style="width: 50px; text-align: center;">#</th>
                  <th style="width: 100px; text-align: center;">User</th>
                  <th style="width: 200px; text-align: center;">Masukan/Pertanyaan</th>
                  <th style="width: 200px; text-align: center;">Respon</th>
                </tr>
              </thead>
              <tbody>
                <?php $helpdesk=$this->Model->read('help_desk'); 
                $no=1;
                if ($helpdesk) {
                  foreach ($helpdesk as $id) { 
                  	if ($id['role']=='mahasiswa') {
                  		$User=$this->Model->read_detail('nim',$id['username'],'mahasiswa');
                  	}elseif ($id['role']=='dosen') {
                  		$User=$this->Model->read_detail('noreg',$id['username'],'dosen');
                  	}else{
                  		$User=$this->Model->read_detail('noreg',substr($id['username'], 2),'dosen');
                  	}
                    ?>
                    <tr>
                      <td align="center" style="vertical-align: middle;"><?= $no;?></td>
                      <td style="width: 100px; vertical-align: middle; text-align: center;">[ <?= $id['username']?> ]<br><?=$User->nama;?></td>
                      ...
                      <?php }else{ ?>
                      	<button data-toggle="modal" data-target=".bs-example-modal-sm_respon<?=$id['id']?>" class="btn btn-primary btn-block"><span class="fa fa-pencil"></span>&nbsp;Tambah Respon</button>
                      <?php	} ?></td>
                      <!-- Small modal -->
                        <div class="modal fade bs-example-modal-sm_respon<?=$id['id']?>" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                <h4 class="modal-title" id="myModalLabel">Tambah Respon</h4>
                              </div>
                              <div class="modal-body col-md-12">
                              <div class="col-md-2"></div>
                              <div class="col-md-8">
                                <form method="POST" action="<?php echo base_url('Staff/respon_help_desk/'.$id['id']); ?>">
                                <div class="form-group col-md-12">
                                  <label for="input">Respon :</label>
                                  <textarea class="form-control" name="respon" required="" placeholder="Silahkan menulis respon di sini"></textarea>
                                </div>
                              </div>
                              <div class="col-md-2"></div>
                              </div>
