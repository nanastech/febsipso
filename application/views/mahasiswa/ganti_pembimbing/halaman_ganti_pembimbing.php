...
<head>
  ...
  <title>SIPSO | Ganti Pembimbing Skripsi</title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a class="active">Ganti Pembimbing Skripsi</a></li>
      </ol>
    </section>
    ...
    <section class="content">
      ...
      <div class="box">
        ...
        <div class="box-body table-responsive">
          <?php 
          $cek = $this->Model->cek_ganti_pembimbing($this->session->userdata('username'));
          ...
          if ($cek && empty($cek->accstaff)) { //jika ada dan belum di acckaprodi dan sudah daftar
            ...
            Tidak bisa mengajukan permohonan ganti pembimbing skripsi, karena permohonan ganti pembimbing skripsi sebelumnya belum divalidasi Subag LAA. Mohon menghubungi Subag LAA.</div>';
          }elseif ($cek && !empty($cek->accstaff) && empty($cek_daftar_sidang)) { ?> <!-- jika ada dan sudah di acckaprodi dan belum daftar sidang -->
            <div class="col-md-12">
              <?php $pembimbing=$this->Model->read_detail('nim',$this->session->userdata('username'),'dospem'); 
              $dosen=$this->Model->read_detail('noreg',$pembimbing->noreg,'dosen'); ?>
              <form action="<?php echo base_url('Mahasiswa/proses_ganti_pembimbing/'.$this->session->userdata('username')); ?>" method="POST">
              <div class="col-md-3"></div>
              <div class="col-md-6">
                <div class="form-group col-md-12">
                  <label class="control-label">Pembimbing Sebelumnya :</label>
                  <select class="form-control" name="pembimbing_lama" readonly required="">
                    <option value="<?=$dosen->noreg;?>">[<?=$dosen->noreg?>] <?=$dosen->nama;?></option>
                  </select>
                </div>
                <div class="form-group col-md-12">
                  <label class="control-label">Pembimbing Baru<font color="red">*</font> :</label>
                  <select class="form-control" name="pembimbing_baru" required="">
                    <option value="">--Pilih--</option>
                    <?php $dosens=$this->Model->read_orderby1('dosen','nama','ASC');
                    foreach ($dosens as $dsn) {
                      echo '<option value="'.$dsn['noreg'].'">['.$dsn['noreg'].'] '.$dsn['nama'].'</option>';
                    }
                     ?>
                  </select>
                </div>
                <div class="form-group col-md-12">
                  <label class="control-label">Keterangan Ganti Pembimbing :</label>
          ...
          <?php }else{
            ...
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
            <h4><i class="icon fa fa-warning"></i> Perhatian!</h4>
            Anda sudah mengajukan sidang skripsi, tidak bisa mengajukan permohonan ganti pembimbing skripsi!</div>';
            } ?>
          
          <div class="col-md-12">
            <legend>Status Permohonan Ganti Pembimbing</legend>
            <table id="example1"  class="table table-bordered table-striped">
              ...
              <tbody>
                <?php $ganti_pembimbing=$this->Model->read_where('nim',$this->session->userdata('username'),'ganti_dospem'); 
                $no=1;
                if ($ganti_pembimbing) {
                      ...
                      <td align="center" style="vertical-align: middle;">
                        ...
                        if (empty($id['accstaff'])) { ?>
                          <a type="button" class="btn btn-primary disabled" > Validate</a>
                           &nbsp;<a onclick="return confirm('Menghapus permohonan pembimbing, apakah anda yakin?');" href="<?= base_url('Mahasiswa/hapus_permohonan_ganti_pembimbing/'.$id['id'])?>" type="button" class="btn btn-danger" ><span class="fa fa-remove"></span></a>
                        <?php }else{
...
<script>
  ...
  var formulir = document.getElementById("ganti_pembimbing");
